...
In its default configuration, Petclinic uses an in-memory database (mysql) which
...
For MySQL database, it is needed to change param "mysql" to "mysql" in string
```
...
For PostgeSQL database, it is needed to change param "mysql" to "postgresql" in string
```
spring.profiles.active=mysql,spring-data-jpa
```
 defined in application.properties file.
